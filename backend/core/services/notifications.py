...
def send_order_status_sms(order, event):
    def _start_thread():
        thread = threading.Thread(
            target=_send_order_status_sms_sync, args=(order, event), daemon=True
        )
        thread.start()
    transaction.on_commit(_start_thread)


def _send_order_status_sms_sync(order, event):
    try:
        ...
        logger.error(f"Unexpected error in background SMS thread for order {order.id}: {e}")
    finally:
        connection.close()
